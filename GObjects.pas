...
begin
  for I := 0 to Points.Count - 1 do
    OwnerDrawing.OnControlPoint(Points[I], VT, ClipRect2D);
end;
...
begin
  ...
  if DeleteControlPoint0(I) > 0 then
    if OwnerDrawing is TDrawing2D then
...
begin
  ...
  if Result and (OwnerDrawing is TDrawing2D) then
    (OwnerDrawing as TDrawing2D).NotifyChanged;
end;
...
begin
  for I := 0 to fPoints.Count - 1 do
    ...
    begin
      ...
      OwnerDrawing.OnControlLine(fPoints[I], TmpPt2, VT,
        ClipRect2D);
    end;
  for I := 0 to fPoints.Count - 1 do
  begin
    if ((I mod 3) = 0) {or (I = fPoints.Count - 1)} then
      OwnerDrawing.OnControlPoint(fPoints[I], VT, ClipRect2D)
    else
      OwnerDrawing.OnControlPoint2(fPoints[I], VT, ClipRect2D);
  end;
end;
