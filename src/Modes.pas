...
type
  ...
  protected
    fLastClickedPoint: TPoint2D;
    fLastMousePoint: TPoint2D;
...
begin
  ...
  OptionsForm.Caption := 'Picture properties';
...
begin
  case Msg of
    ...
    Msg_AddPoint:
      begin
        Add_Point(fLastClickedPoint);
        Drawing.RepaintViewports;
      end;
    Msg_DeletePoint:
      begin
        Delete_Point(fLastClickedPoint);
        Drawing.RepaintViewports;
      end;
    Msg_BreakPath:
      begin
        Break_Path(fLastClickedPoint);
        Drawing.RepaintViewports;
      end;
    ...
    Msg_StartRotate:
      begin
        RotateMode.fLastPoint := fLastMousePoint;
        PushMode(RotateMode);
      end;
...
procedure TBaseMode.OnMouseDown(Sender: TObject; Button:
  ...
  Shift: TShiftState; X, Y: Integer);
begin
  fLastClickedPoint := ViewPort.ScreenToViewport(Point2D(X, Y));
end;
...
var
  ...
  TmpDist: TRealType;
begin
  fLastMousePoint := ViewPort.ScreenToViewport(Point2D(X, Y));
...
//        SelectObjectsInAreaMode.PressedBtn := Sender;
    ...
    begin
      ...
      DragCopyMode.StartPoint :=
        ViewPort.GetSnappedPoint(fLastClickedPoint);
...
procedure TInsertSimpleGrObjMode.OnMouseUp(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
procedure TInsertUnsizedGrObjMode.OnMouseUp(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
  CurrPoint: TPoint2D;
  Prim: TPrimitive2D;
