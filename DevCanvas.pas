...
procedure DrawAsPolygon(PP: TPointsSet2D; const Cnv: TCanvas; const
  Clip, Extent: TRect2D; const S: TTransf2D);
    //TSY:
procedure DrawAsPolygonOutline(PP: TPointsSet2D; const Cnv:
  TCanvas;
  const
  Clip, Extent: TRect2D; const S: TTransf2D);
...
{$IFDEF VER140}
  ...
  EndPath(Cnv.Handle);
  //Windows.Polyline(Cnv.Handle, Points, N);
  //Cnv.Polyline(Slice(Points, N));
{$ELSE}
  Cnv.Polyline(@Pnts, N);
{$ENDIF}
end;
...
{$IFDEF VER140}
  ...
  EndPath(Cnv.Handle);
//  Windows.Polygon(Cnv.Handle, Points, N);
//  Cnv.Polygon(Slice(Points, N));
{$ELSE}
  Cnv.Polygon(Pnts, N);
{$ENDIF}
end;
...
begin
  ...
  begin
    ...
    Brush.Color := clWhite;
    Cnv.Ellipse(X - TmpWdt, Y - TmpWdt, X + TmpWdt, Y + TmpWdt);
//    CnvDrawLine(Cnv, Point2D(X - TmpWdt, Y - TmpWdt),
//      Point2D(X + TmpWdt, Y + TmpWdt));
  end;
end;
...
//    jvTop: AlignFlags := AlignFlags + TA_BOTTOM;
