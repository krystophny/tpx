...
var
  ...
  begin
    ...
    Lines.LoadFromFile('##');
    Lines.Add(aReader.Name);
    Lines.Add(aReader.Value);
    Lines.Add(DecodeHtmlString(aReader.Value));
    Lines.SaveToFile('##');
    Lines.Free;
...
//  aStm.WriteBuffer(Pointer(aXml)^, Length(aXml));
  aStm.WriteBuffer(aXml[1], Length(aXml));
end;
...
begin
  ...
  Move(FData[anOffset + 1], FData[anOffset + anAddLen + 1], aLen
    - anOffset);
  Move(aData[1], FData[anOffset + 1], anAddLen);
end;
