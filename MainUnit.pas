...
uses
  ...
  Forms, Menus, ComCtrls, Clipbrd,
  StdCtrls, ExtCtrls, ImgList, ActnList, ToolWin, ExtDlgs,
  Drawings, ViewPort, GObjects, Manage, Options0, Dialogs,
{$IFDEF VER140}
  Windows, HH, hh_funcs, D6OnHelpFix
{$ELSE}
...
//    procedure LocalModeChanged(Sender: TObject);
...
uses Output, Input, Settings0, ColorEtc, Geometry, Options, Preview,
  SysBasic;
...
begin
  CurrPoint2D := LocalView.GetSnappedPoint(
    LocalView.ScreenToViewport(Point2D(X, Y)));
  StatusBar1.Panels[0].Text := Format('X: %6.3f Y: %6.3f',
...
//* LocalPopUpPopup
//* Better ruler numbers (fractions)
//* Remove MainForm from Modes
//* Show mouse coordinates using a manager when drawing
